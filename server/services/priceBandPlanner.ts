...
export function buildPlannerPrompt(ctx: PlannerContext): string {
  ...
  /*
   * 未保有銘柄には取得単価がないため、価格水準の基準が変わる。
   * 保有銘柄は「取得単価より上か下か」で判断できるが、未保有は基準が無い。
   * そのため 52週レンジ・配当利回り・フェアバリューを基準にするよう明示する。
   * この指示がないと、AI は現在値から機械的に -10% / -20% と刻むだけになり、
   * 「なぜその価格なのか」の根拠がない段組みになる。
   */
  const scopeGuide = ctx.position
    ? `以上の材料に基づいて、この銘柄の買い増し価格帯を 3〜5 段で設計してください。`
    : `以上の材料に基づいて、この銘柄を新規に買う場合の価格帯を 3〜5 段で設計してください。

この銘柄は未保有なので、取得単価という基準がありません。代わりに次を基準にしてください:
- 52週レンジ内の位置（安値圏まで下がったか）
- 配当がある場合、その価格での配当利回り（何 % になれば妥当か）
- 想定フェアバリューとの乖離
- 過去の下落局面での下落幅

また未保有なので、最初の 1 単位を買う水準（初回打診）と、さらに下がったときに
買い増す水準を区別してください。REDUCE の段は作らないでください（保有していないため）。`;
