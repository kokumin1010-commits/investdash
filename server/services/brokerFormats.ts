...
SGX 銘柄では Mkt Value と Unrealized P/L がセント単位まで表示されるため、
その場合は次の式で逆算する。

    取得原価 = Mkt Value − Unrealized P/L
    avgCost  = 取得原価 ÷ 数量

損益がマイナスなら「Mkt Value −（負の損益）」なので取得原価は増える。
損益率は小数第2位に丸められているため、**損益率から逆算してはならない。**
「現在値 × 数量 − 含み損益」も、現在値が遅延値の場合があるため使わない。

日本株の Mkt Value は「JPY 1.59M」のように丸められているため、折り畳みカードから
取得単価を正確に逆算できない。Avg Price が表示されていない日本株は avgCost を null にし、
既存の同一口座・同一数量の保有がある場合は後段で既存取得単価を保持する。
