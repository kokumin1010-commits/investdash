...
const RAKUTEN_ISPEED: BrokerFormat = {
  ...
  layoutPrompt: `このスクリーンショットは楽天証券 iSPEED アプリの「保有銘柄」画面、または「新しいMy Page」の口座サマリー画面である。

## 「新しいMy Page」の口座サマリー

- 「預り金」の明示値は楽天証券口座の実際のJPY現金である。account.cash に入れ、account.currency は "JPY"、account.broker は "楽天証券 iSPEED" とする
- 「評価額合計」「国内株式」「米国株式」「現物評価損益額」は現金ではない。account.cash に入れない
- 「評価額合計」は「純資産」と明記されていないため account.netAssets に入れない
- 画面下部のS&P500指数、日経225、TSLA、AAPL、NCLH等は市況・お気に入りの価格一覧であり、**保有ポジションではない**。保有数量と平均取得価額が表示されていない銘柄は positions に絶対に入れない
- サマリー画面と保有銘柄画面が同じバッチにある場合、預り金はaccountへ、保有は「保有銘柄」画面からだけ抽出する
...
- 「保有銘柄」画面だけには純資産・預り金・現金残高が表示されないため、その画像だけの場合はcashをnullにする。「新しいMy Page」の「預り金」が同じバッチにある場合は上記ルールでaccount.cashへ入れる
...
- **基金タブ** = 貨幣市場基金（現金に近い商品で毎日利息が付く）。
  **株式として取り込んではならない。** 「易方達(香港)美元貨幣市場基金」「平安貨幣基金」
  ...
  基金タブの画像が渡された場合は positions を空配列にし、各商品を interestAssets に入れる。
  商品名、通貨、現在残高／持有金額、昨日収益／日次利息、累計収益／持有收益、表示年率、
  画面に明記された基準日をそれぞれ対応する欄へ入れる。表示のない値は null にする。
  年率から日次収益を逆算したり、現在残高から元本や累計収益を逆算してはならない。
  同じ商品が複数画像に重複している場合は、基準日が同じなら1件にまとめる。
- **債券タブ** も同様に株式ではない。
...
- **港股は 5 桁ゼロ埋め**で表示される（00005 / 00823 / 00883 / 02318）。
  OCR出力の tickerCode には画面に見える **5桁を一字も削らずそのまま**入れる。
  例: 00005→"00005"、00823→"00823"、00883→"00883"、02318→"02318"。
  **右端の数字を削って 0082 / 0088 / 0231 にしてはならない。** exchange は "SEHK" とする。
  Yahoo Finance用の4桁symbol（0005.HK / 0823.HK / 0883.HK / 2318.HK）への変換は後段処理が行うため、OCR段階で変換しない。
...
「Mkt Value − Unrealized P/L」には売却手数料等の影響が混ざる可能性があり、
日本株の Mkt Value は「JPY 1.59M」のように百万単位へ丸められるため使わない。
同一口座・同一数量の既存保有がある場合は、後段で既存の確認済み取得単価を保持する。
