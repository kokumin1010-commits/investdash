...
- [ ] 实现外部健康探测、进程内存采样、崩溃恢复识别和即时通知（代码完成，待生产告警/恢复邮件实测）
- [ ] 实现 112 个持仓 symbol 的真实新闻抓取、symbol 级去重、关联、失败续跑与覆盖率（代码完成，待生产 112 symbol 回填与续跑证据）
- [x] 实现逐标的保有期间、起算依据、精确/至少/记录开始可信度及日文页面展示
- [ ] 将新增新闻自动接入价格带 UNKNOWN 项重新核验并保留来源证据（代码完成，待生产新新闻复核实测）
- [x] 新增可观测性、新闻覆盖、持有期间和价格带复核的服务/路由/页面测试（InvestDash 106 文件、977 项；SalesDash 7 项定向测试）
- [ ] 部署 Railway 并验证告警、内存记录、112 标的新闻回填、持有期间和自动复核
- [ ] 更新运维文档、同步 GitHub 并保存最终检查点
