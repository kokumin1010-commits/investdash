...
Railway 正式版本 `d498829` 已自动执行迁移 `0031_thick_stranger.sql`。首次安全回填生成 **13 个活动待办：REDUCE 10 / WATCH 3**；HOLD 99 未入队，未修改任何持仓或下单。2733.T 快照为当前 2,000 株 → 建议一部売却 500 株（¥1,369,000）→ 执行后 1,500 株/0.4731%；3436.T 为 1,800 株 → 400 株（¥1,373,600）→ 1,400 株/0.5538%。[1]

390px 正式页面截图确认：实际首项完整显示当前 600 株/2.10%/¥18,207,000，一部売却 150 株（概算 ¥4,552,500），执行后 450 株/1.57%；长理由在卡内自然换行，页面 `scrollWidth=clientWidth=390`，无横向溢出。首轮验收在 skeleton 阶段过早判定，脚本改为等待“今回の具体案”后，移动端、1280px 桌面和 Dashboard 入口三种场景全部通过。

1280px 正式截图中，侧栏新增“アクション待ち”，顶部统计显示确认待ち 13、48小时以内 0、计划済み 0、あとで確認 0；ACN 卡清晰呈现 600 株/2.10%/¥1,820.7万 → 一部売却 150 株/¥455.2万 → 450 株/1.57%，并保留“計画に追加／あとで確認／今回は見送る”。390px Dashboard 的 AI 信号卡上方显示绿色入口“アクション待ち 13件”，其下仍保留“今週確認する銘柄 112銘柄”，两种工作流在同一视区内清晰分离。

## References

[1]: https://salesdash.buzzdrop.co.jp/investdash/action-queue "InvestDash 正式アクション待ち页面（2026-08-29 验收）"
